...
minetest.register_globalstep(function(dtime)
    ...
    while i <= #worldedit.queue and i <= worldedit.BLOCKS_PER_GLOBALSTEP do
        ...
        if worldedit.queue[idx].t == "set_node" then
            minetest.set_node(worldedit.queue[idx].pos, worldedit.queue[idx].node)
        elseif worldedit.queue[idx].t == "remove_node" then
            minetest.remove_node(worldedit.queue[idx].pos)
        elseif worldedit.queue[idx].t == "place_node" then
            minetest.place_node(worldedit.queue[idx].pos, worldedit.queue[idx].node)
        elseif worldedit.queue[idx].t == "dig_node" then
            minetest.dig_node(worldedit.queue[idx].pos)
        elseif worldedit.queue[idx].t == "add_entity" then
            minetest.add_entity(worldedit.queue[idx].pos, worldedit.queue[idx].name)
        elseif worldedit.queue[idx].t == "add_item" then
            minetest.add_item(worldedit.queue[idx].pos, worldedit.queue[idx].item)
        elseif worldedit.queue[idx].t == "meta_from_table" then
            minetest.get_meta(worldedit.queue[idx].pos):from_table(worldedit.queue[idx].table)
        else
...
-- the other functions are left out because they are not used in worldedit
    to_table    = function(self) return minetest.get_meta(self._pos):to_table() end,
    set_string  = function(self, name_, value_) minetest.get_meta(self._pos):set_string(name_, value_) end,
...
-- ALL functions that are not just piped to the real minetest.env function must copy the arguments, not just reference them
    ...
    get_node        = function(self, pos) return minetest.get_node(pos) end,
    get_node_or_nil = function(self, pos) return minetest.get_node_or_nil(pos) end,
    get_node_light  = function(self, pos, timeofday) return minetest.get_node_light(pos, timeofday) end,
    place_node      = quene_placenode,
    dig_node        = quene_dignode,
    punch_node      = function(self, pos) return minetest.punch_node(pos) end,
    get_meta        = get_meta_alias,
    get_node_timer  = function(self, pos) return minetest.get_node_timer(pos) end,
