...
function survivalist.register_machine(machine_type, itemtable)
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local inv = meta:get_inventory()
			if inv:is_empty("src") and string.find(minetest.get_node(pos).name, "_on") == nil then return end
...
-- 			if step <= 0 and inv:get_stack("fuel", 1):get_count() > 0 and inv:get_stack("src", 1):get_count() > 0 then
			if ( step <= 0 or rtime <= 0 ) and ( not inv:is_empty("fuel") ) and ( not inv:is_empty("src") ) then
				if ( output == "" and ( check_input(inv:get_stack("src", 1):get_name()) == nil or take_src(inv, meta) == nil ) )
				or ( rtime <= 0 and take_fuel(inv, meta) == nil ) then
			...
			elseif step <= 3 and rtime > 0 then
			...
			ttime = meta:get_int("ttime") or 0
			rtime = meta:get_int("rtime") or 0
			meta:set_string("formspec", default.get_furnace_active_formspec(pos, math.floor(((ttime-rtime)/ttime)*100)))
		end,
	})
end
