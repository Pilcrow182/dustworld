...
--Pilcrow's mese chests
	
	if minetest.get_modpath("chestplus") ~= nil then
		-- check for left/right connects

		if string.find(nxm.name, "chestplus:mese") ~= nil
		and (nxm.param2 == 0 or nxm.param2 == 2) then
			pxm=1 end
		if string.find(nxp.name, "chestplus:mese") ~= nil
		and (nxp.param2 == 0 or nxp.param2 == 2) then
			pxp=1 end

		if string.find(nzm.name, "chestplus:mese") ~= nil
		and (nzm.param2 == 1 or nzm.param2 == 3) then
			pzm=1 end
		if string.find(nzp.name, "chestplus:mese") ~= nil
		and (nzp.param2 == 1 or nzp.param2 == 3) then
			pzp=1 end
		...
		if string.find(nym.name, "chestplus:mese") ~= nil then pym = 1 end
		if string.find(nyp.name, "chestplus:mese") ~= nil then pyp = 1 end

		-- does not scan for the front side of the node.
	end

-- Pilcrow's item crates
	if string.find(nxm.name, "survivalist:crate") ~= nil then pxm=1 end
	if string.find(nxp.name, "survivalist:crate") ~= nil then pxp=1 end
	if string.find(nym.name, "survivalist:crate") ~= nil then pym=1 end
	if string.find(nyp.name, "survivalist:crate") ~= nil then pyp=1 end
