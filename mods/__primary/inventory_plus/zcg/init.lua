...
zcg.add_craft = function(input, output, groups)
	...
	for i, item in pairs(c.items) do
		...
		elseif not minetest.registered_items[item] then -- check for recipes that need non-existant items
			print("DEBUG:: ZCG: '"..item.."' does not exist. Removing a recipe for '"..output.."' from the craft guide.")
			return
		end
	end
