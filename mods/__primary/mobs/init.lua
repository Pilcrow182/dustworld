...
local DUST_MONSTER = true
local TREE_MONSTER = true
local SHEEP = false
local RAT = false
local OERKKI = false
local DUNGEON_MASTER = false

dofile(minetest.get_modpath("mobs").."/api.lua")
if minetest.get_modpath("trees") == nil then minetest.register_alias("trees:jungletree_sapling", "default:junglesapling") end
-- if minetest.get_modpath("pyramids") == nil then minetest.register_alias("pyramids:trap", "default:sandstonebrick") end
...
if DUST_MONSTER then
	mobs:register_mob("mobs:dust_monster", {
		...
		collisionbox = {-0.4, -0.01, -0.4, 0.4, 1.9, 0.4},
		visual = "mesh",
		mesh = "mobs_dust_monster.x",
		textures = {"mobs_dust_monster.png"},
		visual_size = {x=8,y=8},
		makes_footstep_sound = true,
		view_range = 15,
		walk_velocity = 1.5,
