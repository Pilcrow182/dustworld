--Makes evergreen tree
function snow.make_evergreen(pos,snow,xmas)
	local perlin1 = minetest.get_perlin(112,3, 0.5, 150)
	local try_node = function(pos, node)
		local n = minetest.get_node(pos).name
		if  n == "air" or n == "snow:needles" or n == "default:leaves" or n == "snow:sapling_evergreen" or n == "snow:snow" or "snow:needles_decorated" then
	...
	for x=-1,1 do
	for z=-1,1 do
		if minetest.get_node({x=pos.x+x,y=pos.y,z=pos.z+z}).name == "snow:snow" then
			minetest.remove_node({x=pos.x+x,y=pos.y,z=pos.z+z})
		end
		if minetest.get_node({x=pos.x+x,y=pos.y,z=pos.z+z}).name == "snow:snow_block" then
			minetest.remove_node({x=pos.x+x,y=pos.y,z=pos.z+z})
		end
	end
	end
	...
	for i=0, 4 do
		...
		try_node({x=pos.x,y=pos.y+i,z=pos.z},{name="snow:tree"})
	end
...
if maxp.y >= -10 then
		...
		local evergreens = pr:next(1,2) == 1 --spawns evergreens.
		...
		local make_evergreen = snow.make_evergreen
		...
		if alpine then
			local trees = minetest.find_nodes_in_area(minp, maxp, {"default:leaves","default:tree", "default:apple"})
		...
		for i=0,divs do
			...
			local z = z0+j
                ...
                if not plain or pr:next(1,12) == 1 then
					...
					-- Snowy biome stuff
					local node = minetest.get_node({x=x,y=ground_y,z=z})
					...
					if ground_y and node.name == "default:dirt_with_grass" then
							...
							if alpine then
								--Gets rid of dirt
								minetest.add_node({x=x,y=ground_y+1,z=z}, {name="snow:snow",param2=veg})
								for y=ground_y,-6,-1 do
									if minetest.get_node({x=x,y=y,z=z}) and minetest.get_node({x=x,y=y,z=z}).name == "default:stone" then
										break
									else
										minetest.add_node({x=x,y=y,z=z},{name="default:stone"})
									end
								end
							elseif shrubs and pr:next(1,28) == 1 then
								--Spawns dry shrubs.
								minetest.add_node({x=x,y=ground_y,z=z}, {name="snow:dirt_with_snow"})
								if snowy then
									minetest.add_node({x=x,y=ground_y+1,z=z}, {name="snow:snow_block", param2=3})
								else
									minetest.add_node({x=x,y=ground_y+1,z=z}, {name="default:dry_shrub"})
								end
							elseif evergreens and pr:next(1,36) == 1 then
								--Spawns evergreens.
								minetest.add_node({x=x,y=ground_y,z=z}, {name="default:dirt_with_grass"})
								make_evergreen({x=x,y=ground_y+1,z=z},true)
							elseif snowy then
								--Spawns snow blocks.
								minetest.add_node({x=x,y=ground_y+1,z=z}, {name="snow:snow_block"})
								minetest.add_node({x=x,y=ground_y+2,z=z}, {name="snow:snow",param2=veg})
							else
								--Spawns snow.
								minetest.add_node({x=x,y=ground_y,z=z}, {name="snow:dirt_with_snow"})
								minetest.add_node({x=x,y=ground_y+1,z=z}, {name="snow:snow",param2=veg})
					...
					elseif ground_y and node.name == "default:sand" then
						...
						if not icy then
							minetest.add_node({x=x,y=ground_y+1,z=z}, {name="snow:snow"})
							minetest.add_node({x=x,y=ground_y,z=z}, {name="snow:snow_block"})
						else
							minetest.add_node({x=x,y=ground_y,z=z}, {name="snow:ice"})
						end
					elseif ground_y and minetest.get_node({x=x,y=ground_y,z=z}).name == "default:leaves" then
						minetest.add_node({x=x,y=ground_y+1,z=z}, {name="snow:snow"})
					elseif ground_y and minetest.get_node({x=x,y=ground_y,z=z}).name == "default:papyrus" then
						for i=ground_y, ground_y-4, -1 do
							if minetest.get_node({x=x,y=i,z=z}).name == "default:papyrus" then
								minetest.add_node({x=x,y=ground_y+1,z=z}, {name="snow:snow"})
								minetest.add_node({x=x,y=i,z=z}, {name="snow:snow_block", param2=2})
							end
						end
					elseif ground_y and node.name == "default:water_source" then
						if not icesheet and not icecave and not icehole then
							--Coastal ice.
							local x1 = minetest.get_node({x=x+1,y=ground_y,z=z}).name
							local z1 = minetest.get_node({x=x,y=ground_y,z=z+1}).name
							local xz1 = minetest.get_node({x=x+1,y=ground_y,z=z+1}).name
							local xz2 = minetest.get_node({x=x-1,y=ground_y,z=z-1}).name
							local x2 = minetest.get_node({x=x-1,y=ground_y,z=z}).name
							local z2 = minetest.get_node({x=x,y=ground_y,z=z-1}).name
							local y = minetest.get_node({x=x,y=ground_y-1,z=z}).name
							...
							if icecave then
								...
								for y=ground_y-1,-60,-1 do
									if minetest.get_node({x=x,y=y,z=z}) and minetest.get_node({x=x,y=y,z=z}).name ~= "default:water_source" then
										break
									else
					...
					--~ elseif ground_y and node.name == "snow:snow" and node.name ~= "snow:ice" then
						--~ --Abort genaration.
						--~ local name = minetest.get_node({x=x,y=ground_y-1,z=z}).name
