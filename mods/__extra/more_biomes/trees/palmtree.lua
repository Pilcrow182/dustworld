...
abstract_trees.grow_palmtree = function(pos)
  ...
  -- (trunk)
  for v=0,height-4 do minetest.add_node({x = offset.x, y = offset.y + v, z = offset.z}, {name = trunk_node}) end
  offset = {x=offset.x+math.ceil(math.random(3))-2, y=offset.y, z=offset.z+math.ceil(math.random(3))-2}
  for v=height-3,height-2 do minetest.add_node({x = offset.x, y = offset.y + v, z = offset.z}, {name = trunk_node}) end
  offset = {x=offset.x+math.ceil(math.random(3))-2, y=offset.y, z=offset.z+math.ceil(math.random(3))-2}
  local v = height-1
  minetest.add_node({x = offset.x, y = offset.y + v, z = offset.z}, {name = trunk_node})
  -- (leaves)
  local make_leaves = function(pos, l, v)
    for p=-l,l,l*2 do
      minetest.add_node({x = pos.x, y = pos.y + v, z = pos.z + p}, {name = leaves_node})
      minetest.add_node({x = pos.x + p, y = pos.y + v, z = pos.z}, {name = leaves_node})
    end
  end
  minetest.add_node({x = offset.x, y = offset.y + height, z = offset.z}, {name = leaves_node})
