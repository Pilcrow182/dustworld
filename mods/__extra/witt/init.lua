...
minetest.register_globalstep(function(dtime)
    timer = timer + dtime
    if timer >= 0.2 then -- This runs every 0.2 seconds (that is, 5 times per second) rather than every tick (20 times per second)
        timer = 0
...
function handle_tiles(node) -- Return an image of the tile
    ...
    local tiles, resize_string = node.tiles, "^[resize:64x64"
