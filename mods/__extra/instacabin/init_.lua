...
instacabin.build = function(n, value, env, disp, placer)
	...
	for index = 1, count do
		...
		local pt2 = minetest.registered_nodes[entry.name].paramtype2
		if pt2 == "facedir" then
	...
	minetest.register_craftitem("instacabin:"..build, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if pointed_thing.above then
				...
				file:close()
				local count = instacabin.build(n, value, minetest.env, disp, placer)
				itemstack:take_item()
			end
			return itemstack
		end,
...
local craft_list = {
	...
	{"warehouse_large", "vault", "default:steel_ingot"},
