...
local valid_src = function(pos, dstdir, duct_type)
	...
	if dstdir and minetest.get_item_group(node.name, duct_type) > 0 and minetest.get_item_group(node.name, "duct_connect_"..dstdir) == 2 then
		...
		if duct_type == "itemduct" then
			...
			if minetest.get_item_group(node.name, "metastorage") > 0 then return {"meta", "metastorage"} end
			return false
...
local valid_dst = function(pos, duct_type)
	local node = minetest.get_node(pos)
	if not ( node and node.name ) then return false end
	local inv = minetest.get_meta(pos):get_inventory()
	if duct_type == "itemduct" then
...
local metastorage = function(action, pos, stack)
	...
	local metastorage = minetest.get_item_group(node.name, "metastorage")
	if metastorage == 0 then return false end
	...
	if action == "to table" then
		return {name = stored, count = amount, wear = wear}

	elseif action == "to stack" then
	...
	elseif action == "room for item" then
		if stored ~= "" and stored ~= stack:get_name() then return false end
		if amount + stack:get_count() > 9999 then return false end
		if wear ~= 0 and wear ~= stack:get_wear() then return false end
		return true

	elseif action == "add item" then
		if not metastorage("room for item", pos, stack) then return false end
	...
	elseif action == "remove item" then
		if amount < stack:get_count() then return false end
		amount = amount - stack:get_count()
	...
	minetest.register_abm({
		...
		action = function(pos)
			...
			for _,dir in ipairs(dirs) do
				local v = minetest.string_to_pos(vector_dirs[dir])
				local p = {x = pos.x + v.x, y = pos.y + v.y, z = pos.z + v.z}
				if groups["duct_connect_"..dir] == 1 and valid_src(p, nil, basename) then
			...
			local dstpos = find_destination(pos, basename)
			if not dstpos then minetest.log("action", "no destination found") return end

			local src, dst = {}, {}
			for _,srcpos in pairs(inputs) do
				src, dst = valid_src(srcpos, nil, basename), valid_dst(dstpos, basename)
				if src and src[1] == "inventory" then
					local srcinv = minetest.get_meta(srcpos):get_inventory()
					if dst and dst[1] == "inventory" then
						-- Move items from source inventory to destination inventory
						local dstinv = minetest.get_meta(dstpos):get_inventory()
						if not srcinv:is_empty(src[2]) then
							for i,stack in ipairs(srcinv:get_list(src[2])) do
								if not stack:is_empty() then
									local single = {}
									for k,v in pairs(stack:to_table()) do single[k] = v end
									single["count"] = 1
									for count = 1,stack:get_count() do
										if dstinv:room_for_item(dst[2], single) then
--											if dst:allow_metadata_inventory_put() and src:allow_metadata_inventory_take() then --TODO
												dstinv:add_item(dst[2], single)
												srcinv:remove_item(src[2], single)
--												dst:on_metadata_inventory_put() --TODO
--												src:on_metadata_inventory_take() --TODO
--											end
										end
									end
								end
					...
					elseif dst and dst[1] == "meta" then
						-- Move items from source inventory to destination metastorage --TODO
					end
				elseif src and src[1] == "meta" then
					if dst and dst[1] == "inventory" then
						-- Move items from source metastorage to destination inventory --TODO
					elseif dst and dst[1] == "meta" then
						-- Move items from source metastorage to destination metastorage --TODO
					end
				end
			end
		end
