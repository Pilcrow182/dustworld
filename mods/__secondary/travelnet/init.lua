...
minetest.register_privilege("travelnet_attach", { description = "allows to attach travelnet stations to travelnets of other players", give_to_singleplayer = false});
...
-- minetest.chat_send_player(puncher_name, "data: "..minetest.serialize(  travelnet.targets ));
      ...
      meta:set_string("infotext",       "Travelnet Station (unconfigured)");
      ...
      minetest.chat_send_player(puncher_name, "Error: Update failed! Resetting this station on the travelnet.");
      return;
   end
   ...
   local formspec = "size[12,10]"..
                            "label[3.3,0.0;Travelnet-Station:]".."label[6.3,0.0;Punch station to update target list.]"..
...
--                            "button_exit[5.3,0.3;8,0.8;do_update;Punch station to update destination list. Click on target to travel there.]"..
   local x = 0;
   local y = 0;
   local i = 0;
   ...
   minetest.chat_send_player(puncher_name, "The target list of this station on the travelnet has been updated.");
end
...
travelnet.add_target = function( station_name, network_name, pos, player_name, meta, owner_name )
   ...
   elseif( not( minetest.check_player_privs(player_name, {travelnet_attach=true}))) then
      ...
      minetest.chat_send_player(player_name, "You do not have the travelnet_attach priv which is required to attach your station to the network of someone else. Aborting.");
      return;
   end
...
travelnet.on_receive_fields = function(pos, formname, fields, player)
   ...
   -- if the station has not been configured yet
   ...
   -- check if the station has at the other end has been removed
   local node2 = minetest.get_node(  target_pos );
   if( node2 ~= nil and node2.name ~= 'ignore' and node2.name ~= 'travelnet:station' ) then

      -- provide information necessary to identify the removed station
      local oldmetadata = { fields = { owner           = owner_name,
                                       station_name    = fields.target,
                                       station_network = station_network }};

      travelnet.remove_station( target_pos, nil, oldmetadata, player );

   -- do this only on servers where the function exists
   elseif( player.set_look_yaw ) then

      -- rotate the player so that he/she can walk straight out of the station
      local yaw    = 0;
...
travelnet.remove_station = function( pos, oldnode, oldmetadata, digger )
...
--                { -0.2, 0.6,  0.3, 0.2, 1.1,  0.5},
    ...
    tiles = {
             ...
             "travelnet_station_top.png",  -- view from top
             "travelnet_station_top.png",  -- view from bottom
             "travelnet_station_side.png", -- side
             "travelnet_station_side.png", -- side
             "travelnet_station_side.png", -- front view
             "travelnet_station_back.png",  -- backward view
             },
    inventory_image = "travelnet_station.png",
    ...
    after_dig_node = function(pos, oldnode, oldmetadata, digger)
			  travelnet.remove_station( pos, oldnode, oldmetadata, digger )
    end,
    ...
    on_place = function(itemstack, placer, pointed_thing)
       ...
       if( minetest.get_node({x=pos.x, y=pos.y+1, z=pos.z}).name ~= "air" ) then
          ...
          minetest.chat_send_player( placer:get_player_name(), 'Not enough vertical space to place the travelnet station!' )
          return;
       end
...
minetest.register_craft({
        output = "travelnet:station",
        recipe = {
		{"group:wood", "default:mese_crystal", "group:wood", },
		{"group:wood", "group:mesecon_conductor_craftable", "group:wood", },
		{"group:wood", "default:mese_crystal", "group:wood", }
	}
})
